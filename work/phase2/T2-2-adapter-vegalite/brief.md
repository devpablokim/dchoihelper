...
## 목표
차트 엔진 "vegalite" 렌더 어댑터를 구현한다: contracts/chart-request.schema.json 을 만족하는
chart-request.json 을 입력받아 SVG(+PNG)를 렌더하고 contracts/chart-manifest.schema.json 을
만족하는 chart-manifest.json 조각을 산출하는 CLI.
담당 차트 유형: bar, grouped-bar, stacked-bar, line, area, scatter, combo(bar+line)
## 핵심 규약
...
incubator/packages/deck-charts/poc/vegalite/ (렌더 코드 승격 재료), incubator/packages/deck-charts/poc/lib/,
incubator/packages/deck-tokens/presets/vercel/tokens.json (테스트용)
## 산출
allowedWritePaths: incubator/packages/deck-charts/src/adapters/vegalite.mjs,
incubator/packages/deck-charts/src/lib/ (어댑터 공통 유틸 — 다른 어댑터와 겹치면 이 태스크가 먼저 만든 것 우선, 있으면 재사용만),
incubator/packages/deck-charts/tests/run-vegalite.mjs, incubator/packages/deck-charts/tests/requests-vegalite/,
...
expectedCommand: cd incubator && node packages/deck-charts/tests/run-vegalite.mjs
## 금지사항
