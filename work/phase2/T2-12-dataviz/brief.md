...
## 목표
deck-grader 규칙군 2차 중 데이터시각화 규칙군: 3D/파이 과다 금지, 축 절단 왜곡, 데이터-잉크(장식 요소 비율), 범례-직접라벨링, 차트당 시리즈 수 상한 구현. T1-9와 동일 구조 (src/rules/dataviz.mjs + rules-dataviz.yaml 조각).
...
## 산출
allowedWritePaths: incubator/packages/deck-grader/src/rules/dataviz.mjs, incubator/packages/deck-grader/rules-dataviz.yaml, incubator/packages/deck-grader/tests/fixtures/dataviz/, incubator/packages/deck-grader/tests/run-dataviz.mjs
## 합격기준
규칙별 양성/음성 픽스처 + 골든 리포트 스냅샷, 오검출 0. 기존 테스트(run.mjs, run-typo/color/align) 회귀 green.
expectedCommand: cd incubator && node packages/deck-grader/tests/run-dataviz.mjs && node packages/deck-grader/tests/run.mjs
## 공통 금지사항
