...
## 목표
차트 엔진 "graphviz" 한글 골든 렌더 PoC. 방법: @hpcc-js/wasm-graphviz npm (설치됨, 시스템 dot 불필요). Graphviz.load() → dot 렌더 SVG. 조직도형 digraph 1종, 상태도 1종.
...
## 산출
allowedWritePaths: incubator/packages/deck-charts/poc/graphviz/ (렌더 스크립트 render-poc.mjs, out/ SVG·PNG, poc-result.json, 자동검사 스크립트)
## 합격기준
WORKERS.md T2-1 게이트 중 기계분: 한글 렌더 깨짐 0건(자동 검사 green) + poc-result.json 산출 + render-poc.mjs 재실행 가능.
opus PNG 판정은 fable 후속.
expectedCommand: cd incubator && node packages/deck-charts/poc/graphviz/render-poc.mjs && node -e "const r=require('./packages/deck-charts/poc/graphviz/poc-result.json');process.exit(r.status==='pass'&&r.koreanTextOk?0:1)"
## 금지사항
