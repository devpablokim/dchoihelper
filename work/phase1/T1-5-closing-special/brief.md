...
packages/deck-layouts/manifests/closing-special.json (슬롯 bbox·kind·overflowPolicy·capacity는 capacity.mjs로 산출해 기입).
## 입력
...
## 산출
allowedWritePaths: incubator/packages/deck-layouts/templates/closing-special/*.html,
incubator/packages/deck-layouts/manifests/closing-special.json,
incubator/packages/deck-layouts/tests/run-closing-special.mjs, incubator/packages/deck-layouts/tests/content-closing-special/
## 합격기준
...
expectedCommand: cd incubator && node packages/deck-layouts/tests/run-closing-special.mjs
## 금지사항
...
manifests/closing-special.json 외 매니페스트 파일 생성 금지 (통합 매니페스트는 별도 태스크).
